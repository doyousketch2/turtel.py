...
class ScrolledCanvas(TK.Frame):
    ...
    def __init__(self, master, width=500, height=350,
        ...
        self.vscroll.grid(padx=1, in_ = self, pady=1, row=0,
                column=1, rowspan=1, columnspan=1, sticky='ns')
        self.hscroll.grid(padx=1, in_ = self, pady=1, row=1,
                column=0, rowspan=1, columnspan=1, sticky='ew')
        self.reset()
    ...
    def adjustScrolls(self):
        ...
        movetox = 0.5*(self.canvwidth-cwidth)/self.canvwidth
        movetoy = 0.5*(self.canvheight-cheight)/self.canvheight
        self._canvas.xview_moveto( movetox )
        self._canvas.yview_moveto( movetoy )
        if cwidth < self.canvwidth:  ##  only show horiz bars when needed
            print( 'canvwidth', cwidth, movetox )
            self.hscroll.grid(padx=1, in_ = self, pady=1, row=1,
                              column=0, rowspan=1, columnspan=1, sticky='ew')
        else:  ##  hide horiz bars
            print( 'canvwidth', cwidth, movetox )
            self.hscroll.grid_forget()

        if cheight < self.canvheight:  ##  only show vertical bars when needed
            bars = True
            print( 'canvheight', cheight, movetoy )
            self.vscroll.grid(padx=1, in_ = self, pady=1, row=0,
                              column=1, rowspan=1, columnspan=1, sticky='ns')
        else:  ## hide vert bars
            print( 'canvheight', cheight, movetoy )
...
##        """may be implemented for some other graphics toolkit"""
    ...
    def _setscrollregion(self, srx1, sry1, srx2, sry2):
        print('_setscrollregion', srx1, sry1, srx2, sry2)
        self.cv.config(scrollregion=(srx1, sry1, srx2, sry2))

    def _rescale(self, xscalefactor, yscalefactor):
        print('_rescale', xscalefactor, yscalefactor)
...
class TurtleScreen(TurtleScreenBase):
    ...
    def setworldcoordinates(self, llx, lly, urx, ury):
        print('setworldcoordinates', llx, lly, urx, ury)
